...
ny_sweep, nx_sweep = image_deproj.shape
...
# loop through the radius index and make every point in image_sweep at that radius equal to SBr[index]
...
# ### (this is formally extra credit but if you have followed everything above, it should be fairly easy as you just have to do the inverse transformation of the initial step to the residual image)

# In[18]:


image_res = image_deproj-image_sweep
xsweep = isweep*(-disk_delt)
ysweep = jsweep*disk_delt

# function to (linearly) interpolate the map
f = interp2d(blah blah blah, fill_value=0)

image_res_sky = np.zeros(image.shape)
for j in range(ny):
    for i in range(nx):
        x = some function of sky coordinates
        y = some function of sky coordinates
